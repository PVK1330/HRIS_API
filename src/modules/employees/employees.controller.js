...
const getOne = asyncHandler(async (req, res) => {
  ...
  return ApiResponse.ok(res, { employee: emp }, 'Employee retrieved successfully');
});

// POST /api/v1/employees
const create = asyncHandler(async (req, res) => {
  const emp = await service.createEmployee(req.user, req.body);
  return ApiResponse.created(res, { employee: emp }, 'Employee created successfully');
});

// PATCH /api/v1/employees/:id
const update = asyncHandler(async (req, res) => {
  const emp = await service.updateEmployee(req.user, req.params.id, req.body);
  return ApiResponse.ok(res, { employee: emp }, 'Employee updated successfully');
});
