...
async function seedEnterpriseExitWorkflow(dbName) {
  ...
  try {
    ...
    const { rows: wfRows } = await client.query(`
      ...
      VALUES ('Enterprise Employee Exit', NULL, 'Standard 6-stage enterprise exit pipeline', true, true)
      RETURNING id
    `);
    ...
    const stages = [
      ...
      { name: 'IT Clearance', order: 5, mode: 'ANY', sla: 24, dep: 'IT', es_action: 'REASSIGN' },
      { name: 'Administration Clearance', order: 6, mode: 'ANY', sla: 48, dep: 'Administration', es_action: 'REASSIGN' }
    ];
